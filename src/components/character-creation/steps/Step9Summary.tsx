...
const Step9Summary: React.FC = () => {
  ...
  return (
    <div className="space-y-8">
        ...
        {summaryItems.map((item, index) => (
          ...
          >
            {item.value && item.value !== 'None' ? (
              <>
                <div className="h-24 sm:h-32 bg-gray-700 flex items-center justify-center">
                  {item.image ? (
                    <ImageWithFallback
                      src={item.image}
                      alt={item.label}
                ...
                <div className="p-2 sm:p-3 text-center">
                  <h3 className="text-sm sm:text-lg font-semibold text-white truncate">{item.value}</h3>
                  <p className="text-[10px] sm:text-sm text-gray-400">{item.category}</p>
                </div>
                <div className="absolute top-1.5 right-1.5 w-4 h-4 sm:w-5 sm:h-5 bg-primary-500 rounded-full flex items-center justify-center">
              ...
              <div className="h-24 sm:h-32 bg-gray-700 flex items-center justify-center">
                <span className="text-gray-500 text-xs sm:text-sm">Not selected</span>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="text-center">
        <div className="bg-gray-800 rounded-lg p-4 sm:p-6 max-w-2xl mx-auto">
          <h3 className="text-lg sm:text-xl font-semibold mb-4">Character Overview</h3>
          ...
          <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="text-left">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 sm:py-3 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent text-sm sm:text-base"
              />
          ...
          <div className="grid grid-cols-2 gap-4 text-left text-sm sm:text-base">
            <div>
              ...
              <span className="text-white font-medium capitalize">{formData.characterType}</span>
            </div>
            <div>
